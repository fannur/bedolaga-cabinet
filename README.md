...
## Ссылки, открывающие вкладку «Регистрация»

Страница авторизации по умолчанию показывает вкладку «Вход». Переход по ссылкам с
определёнными GET-параметрами автоматически переключает её на «Регистрацию»:

| Ссылка | Результат |
|---|---|
| `https://<domain>/login?tab=register` | Открывается вкладка «Регистрация» |
| `https://<domain>/login?register` | То же (голый ключ `register` без значения) |
| `https://<domain>/login?campaign=max` | Открывается «Регистрация» — **параметр `campaign` с любым значением** (`max`, `max2`, `tg_channel` и т.п.) из рекламных/кампанийных ссылок |
| `https://<domain>/?tab=register` | Неавторизованный пользователь перенаправляется на `/login` **с сохранением query** → открывается «Регистрация» (то же для `?register` и `?campaign=...`) |
| `https://<domain>/login?ref=CODE` | Открывается «Регистрация» (реферальная ссылка, существующее поведение) |

Если в URL есть хотя бы один из параметров `tab=register`, `register` или
`campaign` (любое значение) — вкладка регистрации активна; иначе — по умолчанию
«Вход». Параметры работают и в комбинации, например:
`https://<domain>/login?tab=register&campaign=max2`.

> **Примечание:** слаг `campaign` сохраняется приложением в localStorage
> (TTL 24 часа) и может быть удалён из адресной строки сразу после перехода —
> это штатная механика бонусов кампаний. Вкладка «Регистрация» при этом всё
> равно откроется (проверяется сохранённый слаг), а повторные открытия
> страницы авторизации в течение суток также будут показывать регистрацию.
